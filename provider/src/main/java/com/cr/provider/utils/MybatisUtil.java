 ...
 *
 * @author YAOGANG 2016年12月15日 下午1:19:39
 */
public class MybatisUtil {
     ...
     * 指定表名，不指定则默认跑所有表
     */
    private String specifyTableName = "t_role";
    ...
    private final String base_path_java = "D:/workspaces/ModelProject/provider/src/main/java/";

    private final String base_path_resources = "D:/workspaces/ModelProject/provider/src/main/resources/";

    private final String bean_path = base_path_java + "com/cr/provider/model";

    private final String bean_vo_path = base_path_java + "com/cr/provider/vo";

    private final String mapper_path = base_path_java + "com/cr/provider/mapper";

    private final String controller_path = base_path_java + "com/cr/provider/controller";

    private final String service_path = base_path_java + "com/cr/provider/service";

    private final String service_impl_path = base_path_java + "com/cr/provider/service/impl";

    private final String xml_path = base_path_resources + "mapper";

    private final String moduleName = "provider"; // 对应模块名称（根据自己模块做相应调整!!!务必修改^_^）

    private final String bean_package = "com.cr." + moduleName + ".model";

    private final String bean_vo_package = "com.cr." + moduleName + ".vo";

    private final String mapper_package = "com.cr." + moduleName + ".mapper";

    private final String controller_package = "com.cr." + moduleName + ".controller";

    private final String service_package = "com.cr." + moduleName + ".service";

    private final String service_impl_package = "com.cr." + moduleName + ".service.impl";

    private final String driverName = "com.mysql.jdbc.Driver";

    private final String user = "chuangren";

    private final String password = "Chuangren123";

    private final String url = "jdbc:mysql://rm-uf6sprdnt35233pip3o.mysql.rds.aliyuncs.com:3306/cr_pro?useUnicode=true&characterEncoding=utf8&allowMultiQueries=true";

    private String tableName = null;

    private String beanName = null;

    private String beanVOName = null;
    ...
    private void processTable(String table) {
        ...
        for (int i = n; i < tables.length; i++) {
            temp = tables[i].trim();
            sb.append(temp.substring(0, 1).toUpperCase()).append(temp.substring(1));
        }
        beanName = sb.toString();
        beanVOName = beanName + "VO";
     ...
     * @throws IOException
     */
    private void buildEntityVOBean(List<String> columns, List<String> types, List<String> comments, String tableComment,
                                   String tableName) throws IOException {
        File folder = new File(bean_vo_path);
        if (!folder.exists()) {
            folder.mkdir();
        }

        File beanFile = new File(bean_vo_path, beanVOName + ".java");
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(beanFile)));
        bw.write("package " + bean_vo_package + ";");
        bw.newLine();
        bw.newLine();
        bw.write("import java.io.Serializable;");
        bw.newLine();
        ...
        bw.newLine();
        bw.newLine();
        bw.write("public class " + beanVOName + " implements Serializable {");
        bw.newLine();
        bw.newLine();
        int size = columns.size();
        for (int i = 0; i < size; i++) {
    ...
    private void buildController() throws IOException {
        ...
        bw.newLine();
        bw.write("import com.cr.core.common.ResultType;");
        bw.newLine();
        ...
        bw.write("\t@Autowired");
        bw.newLine();
        bw.write("\tprivate " + serviceName + " " + toLowerCaseFirstOne(serviceName) + ";");
        bw.newLine();
        bw.newLine();
        bw.write("\t@RequestMapping(value = \"/mapping\", method = {RequestMethod.GET})");
        bw.newLine();
        bw.write("\tpublic Map<String, Object> methedName(@RequestParam(\"a\") String a) throws Exception {");
        bw.newLine();
        bw.write("\t\treturn ResultType.getSuccessMap(null);");
        bw.newLine();
        bw.write("\t}");
        bw.newLine();
        bw.newLine();
    ...
    private void buildService() throws IOException {
        ...
        bw.newLine();
        bw.newLine();
        bw.write("\tMap<String, Object> methedName(" + beanName + " " + toLowerCaseFirstOne(beanName) + ") throws Exception;");
        bw.newLine();
        bw.newLine();
        bw.write("\tList<" + beanName + "> selectAll() throws Exception;");
        bw.newLine();
        bw.newLine();
        bw.write("}");
        bw.flush();
    ...
    private void buildServiceImpl() throws IOException {
        ...
        File serviceImplFile = new File(service_impl_path, serviceImplName + ".java");
        ...
        bw.write("import " + service_package + "." + serviceName + ";");
        bw.newLine();
        bw.write("import com.cr.core.common.Constent;");
        bw.newLine();
        bw.write("import com.cr.core.common.ResultType;");
        bw.newLine();
        bw.write("import org.springframework.beans.factory.annotation.Autowired;");
        ...
        bw.write("\t@Autowired");
        bw.newLine();
        bw.write("\tprivate " + mapperName + " " + toLowerCaseFirstOne(mapperName) + ";");
        bw.newLine();
        bw.newLine();
        bw.write("\t@Override");
        bw.newLine();
        bw.write("\tpublic Map<String, Object> methedName(" + beanName + " " + toLowerCaseFirstOne(beanName) + ") throws Exception {");
        bw.newLine();
        bw.write("\t\treturn ResultType.getSuccessMap(null);");
        bw.newLine();
        bw.write("\t}");
        ...
        bw.write("\t@Override");
        bw.newLine();
        bw.write("\tpublic List<" + beanName + "> selectAll() throws Exception {");
        bw.newLine();
     ...
     * @throws IOException
     */
    private void buildTkMapper() throws IOException {
        File folder = new File(mapper_path);
        if (!folder.exists()) {
            folder.mkdirs();
        }
    ...
    private void buildSQL(BufferedWriter bw, List<String> columns, List<String> types) throws IOException {
        ...
        bw.newLine();
        bw.write("\t<select id=\"selectByPrimaryKey\" resultType=\"" + processResultMapId(beanName)
        ...
        bw.newLine();
        bw.write(
                "\t<update id=\"updateByPrimaryKeySelective\" parameterType=\"" + processResultMapId(beanName) + "\">");
        bw.newLine();
     ...
     * @throws IOException
     */
    private void buildTkMapperXml(List<String> columns, List<String> types, List<String> comments) throws IOException {
        File folder = new File(xml_path);
        if (!folder.exists()) {
            folder.mkdirs();
        }
    ...
    private void buildTkSQL(BufferedWriter bw, List<String> columns, List<String> types) throws IOException {
        int size = columns.size();
        // 通用结果列
        bw.write("\t<!-- 通用查询结果列-->");
        bw.newLine();
    ...
    public void generate() throws ClassNotFoundException, SQLException, IOException {
        ...
        List<String> tables = new ArrayList<String>();
        if (StringUtils.isNotEmpty(specifyTableName)) {
            tables.add(specifyTableName);
        } else {
            tables = getTables();
        }
        ...
        for (String table : tables) {
            ...
            buildEntityVOBean(columns, types, comments, tableComment, tableName);
            buildTkMapper();
            buildController();
            buildService();
            buildServiceImpl();
            buildTkMapperXml(columns, types, comments);
        }
        conn.close();
    }
